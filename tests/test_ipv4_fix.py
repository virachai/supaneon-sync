...
class TestIPv4Fix(unittest.TestCase):
    ...
    def test_resolve_to_ipv4(self, mock_getaddrinfo):
        ...
        mock_getaddrinfo.assert_called_with("example.com", None, backup.socket.AF_INET, backup.socket.SOCK_STREAM)

    @patch("supaneon_sync.backup.socket.getaddrinfo")
    def test_resolve_to_ipv4_failure(self, mock_getaddrinfo):
        # Mock failure
        mock_getaddrinfo.side_effect = backup.socket.gaierror("Name or service not known")
    ...
    def test_run_sets_pghostaddr(self, mock_validate_env, mock_connect, mock_getaddrinfo, mock_popen):
        # Setup mock config
        mock_cfg = MagicMock()
        mock_cfg.supabase_database_url = "postgres://user:pass@supabase.host:5432/db?sslmode=require"
        mock_cfg.neon_database_url = "postgres://user:pass@neon.host:5432/db?sslmode=require"
